...
if [[ $startstep -le 7 ]]; then
    ...
    # No longer doing it this way (i.e. directly with the wheel). Could be turned back into an option.
    # But houston has to build using the package - we are no longer building images locally.
...
#    docker build --no-cache \
    ...
    if [[ $status -eq 0 ]]; then
      ...
      # Now this local image will only be for local testing; no longer building and pushing to ECR
